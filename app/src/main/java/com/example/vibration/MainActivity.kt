...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonUp.setOnTouchListener(View.OnTouchListener { v, event ->
            //binding.textView.text = event.x.toString() + " " + binding.buttonUp.layoutParams.width.toString() + " " + event.y.toString() + " " + binding.buttonUp.layoutParams.height.toString()
            ...
            when(direction) {
                0 -> {
                    ...
                    if (event.x > binding.buttonUp.layoutParams.width / 3 && event.x < binding.buttonUp.layoutParams.width * 2 / 3){
                ...
                1 -> {
                    if (((event.x > binding.buttonUp.layoutParams.width / 3 && event.x < binding.buttonUp.layoutParams.width * 2 / 3)
                                && event.y > binding.buttonUp.layoutParams.height / 3)
                        || ((event.y < binding.buttonUp.layoutParams.height / 3)
                                &&(event.x < binding.buttonUp.layoutParams.width * 2 / 3))){
                        val vibrator = (getSystemService(Service.VIBRATOR_SERVICE) as Vibrator).also {
                            it.vibrate(vib)
                        };  //设置手机振动
                        binding.textView.text = "vibrating"
                    ...
                    if (((event.x > binding.buttonUp.layoutParams.width / 3 && event.x < binding.buttonUp.layoutParams.width * 2 / 3)
                                && event.y > binding.buttonUp.layoutParams.height / 3)
                        || ((event.y < binding.buttonUp.layoutParams.height / 3)
                                &&(event.x > binding.buttonUp.layoutParams.width / 3))){
                        val vibrator = (getSystemService(Service.VIBRATOR_SERVICE) as Vibrator).also {
                            it.vibrate(vib)
                        }  //设置手机振动
                        binding.textView.text = "vibrating"
